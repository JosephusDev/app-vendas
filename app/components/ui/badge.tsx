...
function Badge({ className, variant, asChild, onPress, ...props }: BadgeProps & { onPress?: () => void }) {
  const Component = asChild ? Slot.View : View;
  return (
    <TextClassContext.Provider value={badgeTextVariants({ variant })}>
      <TouchableOpacity onPress={onPress}>
        <Component className={cn(badgeVariants({ variant }), className)} {...props} pointerEvents="none"/>
      </TouchableOpacity>
